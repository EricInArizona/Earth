...
namespace ClimateModel
{
  ...
  internal void SetUTCTime( int Year,
                            int Month,
                            int Day,
                            int Hour,
                            int Minute,
  ...
  internal double GetSecondsDifference( DateTime ToTime )
    {
    TimeSpan TimeDif = ToTime.Subtract( UTCTime );
    return TimeDif.TotalSeconds;
    }
  ...
  */
  ...
  internal string GetLocalMillisecondTime()
    ...
    string HourStr = RightNow.Hour.ToString();
    string MinuteStr = RightNow.Minute.ToString();
    string SecondStr = RightNow.Second.ToString();
    string MillisecStr = RightNow.Millisecond.ToString();
    ...
    if( MillisecStr.Length == 2 )
      MillisecStr = "0" + MillisecStr;

    string Result = HourStr + ":" +
                    MinuteStr + ":" +
                    SecondStr + ":" +
                    MillisecStr;
  ...
  internal string ToCrudeString()
    {
    return UTCTime.ToShortDateString() + "  " +
           UTCTime.ToShortTimeString();
